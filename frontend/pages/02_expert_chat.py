...
# ---------------------------------------------------------
st.set_page_config(page_title="무한상사 회의 거버넌스 시스템 - 하이브리드 검증/자문", layout="wide", initial_sidebar_state="expanded")
...
def stream_expert_answer(meeting_id: str, mode: str, query: str):
    ...
    payload = {"meeting_id": meeting_id, "mode": mode, "prompt": query, "include_past_db": False}
    try:
...
# Main Content Area
# ---------------------------------------------------------
st.markdown('<div class="page-title">하이브리드 회의 검증 및 전문가 자문 시스템</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle-text">공식 의사결정의 엄격한 사실 검증과 도메인 전문가의 다각도 자문을 동시에 지원합니다.</div>', unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    with col_mode:
        selected_mode = st.radio(
            "시스템 작동 모드 선택", 
            ["공식 검증 모드 (사실 확인 및 근거 추적)", "전문가 자문 모드 (개념 학습 및 해결책 제시)"], 
            horizontal=True, 
            label_visibility="collapsed"
        )
        st.session_state.system_mode = "검증 모드" if "검증" in selected_mode else "자문 모드"
        
    with col_meet:
        meeting_dict = get_real_meeting_list()
        meeting_options = list(meeting_dict.keys())
        
        # [UX 핵심] 방금 분석한 회의가 있다면 드롭다운의 기본값(index)으로 자동 포커스
        default_index = 0
        if "meeting_id" in st.session_state and st.session_state.meeting_id:
            for i, meeting_id in enumerate(meeting_dict.values()):
                if meeting_id == st.session_state.meeting_id:
                    default_index = i
                    break

        selected_title = st.selectbox("기준 대화 맥락 (회의 단위)", options=meeting_options, index=default_index, label_visibility="collapsed")
        current_id = meeting_dict[selected_title]
        
        # 회의가 변경되면 채팅 히스토리 초기화
    ...
    if prompt:
        ...
        with chat_container:
            ...
            with st.chat_message("assistant"):
                stream_gen = stream_expert_answer(st.session_state.current_meeting, st.session_state.system_mode, prompt)
                full_response = st.write_stream(stream_gen)
                
        st.session_state.hybrid_logs.append({"role": "assistant", "content": full_response})

# ----------------------------------------
# RIGHT COLUMN: Source Attribution Panel
# ----------------------------------------
with col_right:
    st.markdown('<div class="panel-title">참조 데이터 베이스 (RAG Sources)</div>', unsafe_allow_html=True)
    
    with st.container(border=True):
        if len(st.session_state.hybrid_logs) > 0 and current_id:
            mode_color = "#4ADE80" if st.session_state.system_mode == "검증 모드" else "#FBBF24"
            st.markdown(f"""
            <div style="background-color: #0F172A; padding: 15px; border-radius: 6px; border: 1px solid #334155; margin-bottom: 15px;">
                ...
                AI가 <b>{selected_title}</b>의 내용을 Chroma DB에서 벡터 검색(Vector Search)하여 실시간으로 답변을 스트리밍했습니다.
                </span>
            </div>
            ...
            <div class="ctx-card" style="border-left-color: #3B82F6;">
                ...
                <div class="ctx-text">AI가 해당 회의록 내에서 질문과 가장 의미적으로 유사한 텍스트 조각(Chunk)들을 추출하여 답변의 근거(Context)로 활용했습니다.</div>
            </div>
            """, unsafe_allow_html=True)
        else:
